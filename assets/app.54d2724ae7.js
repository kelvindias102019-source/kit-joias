!function(f,b,e,v,n,t,s)
  ...
  fbq('track', 'ViewContent', {
    ...
    const metaMoneyValue=(qty,variant=null)=>{
      const resolvedVariant=variant || sessionStorage.getItem('brbbCartVariant') || 'sem-maleta';
      const withCase=resolvedVariant==='com-maleta';
      const unit=withCase?67.90:57.98;
      const max=withCase?1:3;
    ...
    let selectedVariant='';
    const variantToIndex={'com-maleta':1,'sem-maleta':2};
    ...
    const priceConfig={
      'sem-maleta':{promo:57.98,regular:65.87,maxQty:3},
      'com-maleta':{promo:67.90,regular:99.98,maxQty:1}
    };
    const currentVariant=()=>selectedVariant==='com-maleta'?'com-maleta':'sem-maleta';
    ...
    const applyVariantSelection=()=>{
      if(currentVariant()==='com-maleta') q=1;
      updateQuantityPrice();
    };
    ...
    addToCart.addEventListener('click',()=>{
      ...
      cartLoadingTimer=setTimeout(()=>{
        ...
        metaTrack('AddToCart',{
          content_name:'Kit 104 SemiJoias - '+(cartVariant==='com-maleta'?'Com maleta':'Sem maleta'),
          content_type:'product',
          content_ids:['kit-104-semijoias'],
          currency:'BRL',
  ...
  (() => {
    ...
    const refreshTotals=()=>{
      const variant=sessionStorage.getItem('brbbCartVariant')||'sem-maleta';
      const withCase=variant==='com-maleta';
      const maxQty=withCase?1:3;
      const qty=Math.max(1,Math.min(maxQty,Number(sessionStorage.getItem('brbbCartQty')||1)));
      const unit=withCase?67.90:57.98;
      ...
      if(productImage){
        productImage.src=withCase?'assets/opcao-maleta.png':'assets/opcao-sem-maleta.png';
        productImage.alt=withCase?'Kit com maleta':'Kit sem maleta';
      }
      if(productName){
        productName.textContent=withCase
          ? 'Kit 104 SemiJoias Completo - Com Maleta'
          : 'Kit 104 SemiJoias Completo - Sem Maleta';
      }
      if(variantText) variantText.textContent='Opção: '+(withCase?'Com maleta':'Sem maleta');
      if(couponBadge) couponBadge.textContent=withCase?'32% OFF':'12% OFF';
    ...
    window.openIntegratedCheckout=()=>{
      ...
      refreshTotals();
      const metaCheckoutVariant=sessionStorage.getItem('brbbCartVariant')||'sem-maleta';
      const metaCheckoutMax=metaCheckoutVariant==='com-maleta'?1:3;
      const metaCheckoutQty=Math.max(1,Math.min(metaCheckoutMax,Number(sessionStorage.getItem('brbbCartQty')||1)));
      metaTrack('InitiateCheckout',{
        content_name:'Kit 104 SemiJoias - '+(metaCheckoutVariant==='com-maleta'?'Com maleta':'Sem maleta'),
        content_type:'product',
        content_ids:['kit-104-semijoias'],
        currency:'BRL',
    ...
    document.getElementById('coPayPix').addEventListener('click',()=>{
      ...
      const checkoutVariant=sessionStorage.getItem('brbbCartVariant')||'sem-maleta';
      const withCase=checkoutVariant==='com-maleta';
      const checkoutQty=withCase
        ? 1
        : Math.max(1,Math.min(3,Number(sessionStorage.getItem('brbbCartQty')||1)));

      const pixCheckoutByQty={
        1:'https://pay.veopag.com/shopeepagamentos-ltda-808843',
        2:'https://pay.veopag.com/shopeepagamentos-ltda-f45236',
        3:'https://pay.veopag.com/shopeepagamentos-ltda-5ab304'
      };
      const pixUrl=withCase
        ? 'https://pay.veopag.com/shopeepagamentos-ltda-70c2d5'
        : (pixCheckoutByQty[checkoutQty]||pixCheckoutByQty[1]);

      metaTrack('AddPaymentInfo',{
        content_name:'Kit 104 SemiJoias - '+(withCase?'Com maleta':'Sem maleta'),
        content_type:'product',
        content_ids:['kit-104-semijoias'],
        currency:'BRL',
